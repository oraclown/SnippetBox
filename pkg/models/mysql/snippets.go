...
func (m *SnippetModel) Insert(title, content, expires string) (int, error) {
	// Write the SQL statement wew want to execute. I've split it over two lines
	// for readablility (which is why it's surrounded with backquotes instead
	// of normal double quotes).
	stmt := `INSERT INTO snippets (title, content, created, expires)
	VALUES(?, ?, UTC_TIMESTAMP(), DATE_ADD(UTC_TIMESTAMP(), INTERVAL ? DAY))`

	// Use the Exec() method on the embedded connection pool to execute the
	// statement. The first parameter is the SQL statment, followed by the
...
func (m *SnippetModel) Latest() ([]*models.Snippet, error) {
	// Write the SQL statemet we want to execute.
	stmt := `SELECT id, title, content, created, expires FROM snippets
	...
	// We defer rows.Close() to ensure the awl.Rows resultset is
